...
# Welcome Message
welcome() {
    echo "*****Welcome to ironbot setup*****
This Guided Setup shall help you get your own ironbot up and running.
...
botclone() {
    ...
    if [ -z "$bot_clone" ]; then
        git clone https://github.com/mabotsss/ironbot -b master
        save_prog "bot_clone"
    fi
    echo "DONE!!"
    cd Telegram-UserBot
...
systemd() {
    echo "Sys service..."
    if [ -z "$sysserv" ]; then
        sudo mv ironbot /etc/systemd/system/ironbot.service
        save_prog "sysserv"
    fi
    sudo systemctl start ironbot.service
    sudo systemctl enable ironbot.service
    echo "DONE!!"
}
