...
from ironbot.utils.exceptions import CancelProcess
from ironbot.modules.aria import aria2, check_metadata
# =========================================================== #
#                          STATIC                             #
# =========================================================== #
...
if __ is not None:
    if "uc?id=" in G_DRIVE_FOLDER_ID:
        LOGS.info(
            "folder G_DRIVE_FOLDER_ID tidak valid...")
...
async def generate_credentials(gdrive):
    if helper.get_credentials(str(gdrive.from_id)) is not None:
        await gdrive.edit("`Anda sudah mendapatkan TOKEN...`")
    ...
    if G_DRIVE_DATA is not None:
        try:
            ...
            await gdrive.edit(
                "`[AUTHENTICATE - RUSAK]`\n\n"
                "`Status` : **BAD**\n"
                "`Karena` : **G_DRIVE_DATA** entity kosong!"
            )
            return False
    else:
        if G_DRIVE_CLIENT_ID is None and G_DRIVE_CLIENT_SECRET is None:
            await gdrive.edit(
                "`[AUTHENTICATE - ERROR]`\n\n"
                "`Status` : **BAD**\n"
                "`Karena` : pliss isi dulu **G_DRIVE_DATA** "
    ...
    msg = await gdrive.respond(
        "`Pergi ke BOTLOG group anda untuk mengauthentikasi acess token...`"
    )
...
async def create_app(gdrive):
    ...
    if not creds or not creds.valid:
        if creds and creds.expired and creds.refresh_token:
            await gdrive.edit("`Memperbarui kredensial...`")
            creds.refresh(Request())
            helper.save_credentials(str(gdrive.from_id),
                                    base64.b64encode(pickle.dumps(creds)).decode())
        else:
            await gdrive.edit("`Kredensial masih kosong wkwk , ambil dulu baru di auth...`")
            return False
...
async def reset_credentials(gdrive):
    await gdrive.edit("`Menyetel ulang informasi...`")
    helper.clear_credentials(str(gdrive.from_id))
    await gdrive.edit("`Beress...`")
...
async def download(gdrive, service, uri=None):
    ...
    if uri:
        ...
        try:
            ...
            reply += (
                "`[FILE - BATAL]`\n\n"
                "`Status` : **OK** - gagal menerima sinyal."
            )
            return reply
        else:
...
async def download_gdrive(gdrive, service, uri):
    ...
    try:
            ...
            with io.FileIO(file_path, 'wb') as files:
                ...
                for chunk in download.iter_content(CHUNK_SIZE):
                    ...
                    current_message = (
                        ...
                        f" @ {humanbytes(speed)}`\n"
                        f"`KURANG LEBIH` -> {time_formatter(eta)}"
                    )
                    if round(
        ...
        if mimeType == 'application/vnd.google-apps.folder':
            await gdrive.edit("`Dibatalkan, folder tidak ada...`")
            return False
        file_path = TEMP_DOWNLOAD_DIRECTORY + file_name
    ...
    msg = await gdrive.respond("`Jawab pertanyaan di BOTLOG group`")
    async with gdrive.client.conversation(BOTLOG_CHATID) as conv:
...
async def change_permission(service, Id):
    ...
    try:
        ...
        if f'"File tidak ada: {Id}."' in str(e) or (
            '"Berbagi folder yang ada di dalam drive bersama tidak didukung."'
                in str(e)):
            return
        else:
            raise e
...
async def upload(gdrive, service, file_path, file_name, mimeType):
    try:
        await gdrive.edit("`Proses mengunggah...`")
    except Exception:
        pass
    body = {
        "name": file_name,
        "description": "Diunggah dari telegram dengan Project ironbot.",
        "mimeType": mimeType,
    }
    try:
        if parent_Id is not None:
    ...
    if checker is not None:
        ...
        if page_size > 1000:
            await gdrive.edit(
                "`[GDRIVE - LIST]`\n\n"
                "`Status` : **BAD**\n"
                "`Karena` : tidak mendapatkan list lebih dari 1000."
            )
            return
    else:
...
async def google_drive(gdrive):
    ...
    if not value and not gdrive.reply_to_msg_id:
        ...
        await gdrive.edit(
            "`[UNKNOWN - ERROR]`\n\n"
            "`Status` : **failed**\n"
            "`Karena` : Confused to upload file or the replied message/media."
        )
        return None
    service = await create_app(gdrive)
    if service is False:
    ...
    if isfile(value):
        ...
        if not uri and not gdrive.reply_to_msg_id:
            await gdrive.edit(
                "`[VALUE - ERROR]`\n\n"
                "`Status` : **BAD**\n"
                "`Karena` : value yang diberikan bukanlah URL atau path file / folder. "
                "Jika menurut Anda ini salah, mungkin Anda menggunakan .gd dengan beberapa kali "
                "value dari files/folders, contoh `.gd <filename1> <filename2>` "
            )
            return False
...
async def set_upload_folder(gdrive):
    ...
    if exe == "rm":
        if G_DRIVE_FOLDER_ID is not None:
            ...
            await gdrive.edit(
                "`[FOLDER - SET]`\n\n"
                "`Status` : **OK** - pakai `G_DRIVE_FOLDER_ID` sekarang."
            )
            return None
        else:
            try:
                ...
                await gdrive.edit(
                    "`[FOLDER - SET]`\n\n"
                    "`Status` : **BAD** - tidak ada parent_Id yang di setel."
                )
                return False
            else:
                await gdrive.edit(
                    "`[FOLDER - SET]`\n\n"
                    "`Status` : **OK**"
                    " - `G_DRIVE_FOLDER_ID` kosong, akan menggunakan root folder."
                )
                return None
    ...
    try:
        ...
        if True in [c1 or c2]:
            ...
            await gdrive.edit(
                "`[PARENT - FOLDER]`\n\n"
                "`Status` : **OK** - Berhasil dirubah."
            )
            return None
        else:
            await gdrive.edit(
                "`[PARENT - FOLDER]`\n\n"
                "`Status` : **WARNING** - penggunaan paksa...."
            )
            parent_Id = inp
    else:
        if "uc?id=" in ext_id:
        ...
        try:
            if not complete and not file.error_message:
                ...
                msg = (
                    ...
                    f"`KURANG LEBIH` -> {file.eta_string()}\n"
                )
            ...
            if " depth exceeded" in str(e):
                ...
                try:
                    await gdrive.edit(
                        ...
                        f"`{file.name}`\n"
                        "`Status` : **failed**\n"
                        "`Karena` : Unduhan dibatalkan otomatis, URL / Torrent mati."
                    )
                except Exception:
                    pass
...
CMD_HELP.update({
    "gdrive":
    "`.gdauth`"
    "\nUsage: Buat token untuk mengaktifkan semua layanan cmd google drive."
    ...
    "\nUntuk drivelink."
    "\n\n`.gdabort`"
    "\nUsage: Batalkan proses mengunggah atau mengunduh."
    "\n\n`.gdlist`"
