    ...
    if new_pack:
        ...
        async with bot.conversation("Stickers") as conv:
            ...
            if "Sorry, the image dimensions are invalid." in kontrol.text:
                await event.edit("`Stiker ditolak. Mencoba metode kedua...`")
                try:
                    await bot.send_file("@ezstickerbot", message, force_document=True)
                except YouBlockedUserError:
                    return await event.edit("`buka blokir dan coba lagi` @EzStickerBot `!`")
...
CmdHelp('stickers').add_command(
    'kang', None, 'Dengan kang, Anda dapat membalas stiker atau gambar dan menambahkannya sebagai stiker ke paket stiker Anda sendiri..'
).add_command(
    'kang', '<emoji(s)>', 'Ini berfungsi seperti kang ke stiker, tetapi ini dapat menentukan emoji yang Anda inginkan sebagai emoji stiker..'
).add_command(
    'kang', '<jumlah>', 'Menambahkan stiker atau gambar ke paket yang ditentukan tetapi menggunakan yang berikut ini sebagai emoji: 🤔 '
).add_command(
    'dızla', '<emoji(s)> <numara>', 'Melampirkan stiker atau gambar ke paket yang ditentukan dan digunakan sebagai emoji stiker emoji yang Anda tentukan..'
).add()
