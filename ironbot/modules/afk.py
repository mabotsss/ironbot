...
from ironbot import (AFKREASON, COUNT_MSG, CMD_HELP, ISAFK, BOTLOG,
...
def time_formatter(seconds, short=True):
    ...
    tmp = ((str(days) + (" hari, " if not short else "h, ")) if days else "") + \
        ((str(hours) + (" jam, " if not short else "j, ")) if hours else "") + \
        ((str(minutes) + (" menit, " if not short else "m, ")) if minutes else "") + \
        ((str(seconds) + (" detik, " if not short else "d, ")) if seconds else "")
    return tmp[:-2] + " yang lalu"
    ...
    if ISAFK:
        ...
        if BOTLOG:
            await notafk.client.send_message(
                BOTLOG_CHATID,
                "Saat Anda AFK" + str(len(USERS)) + " orang untuk Anda " +
                str(COUNT_MSG) + " mengirim pesan.",
            )
            for i in USERS:
