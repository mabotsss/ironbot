...
from ironbot import CMD_HELP, bot, tgbot, PLUGIN_CHANNEL_ID, PATTERNS
from ironbot.events import register
from ironbot.main import extractCommands
...
async def pport(event):
    ...
    if re.search(borg1, dosy):
        ...
        degistir = degistir.replace("from ironbot.utils import admin_cmd", "from ironbot.events import register")
        degistir = re.sub(r"(from uniborg).*", "from ironbot.events import register", degistir)
...
async def pins(event):
    ...
    await event.edit(LANG["DOWNLOADING"])
    dosya = await event.client.download_media(reply_message, "./ironbot/modules/")
    ...
    try:
        ...
        return os.remove("./ironbot/modules/" + dosya)
    ...
    if re.search(r"@tgbot\.on\(.*pattern=(r|)\".*\".*\)", dosy):
        ...
        if (not type(Pattern) == list) or (len(Pattern) < 1 or len(Pattern[0]) < 1):
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                return await event.edit(f'**Modul berhasil dimuat!**\n__Untuk informasi tentang perintah dan penggunaan modul__ `.xscr {cmdhelp}` __menulis.__')
            else:
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
                ironbot.cmdhelp.CmdHelp(dosya).add_warning('Perintah tidak ditemukan!').add()
            ...
            if re.search(r'CmdHelp\(.*\)', dosy):
                ...
                await reply_message.forward_to(PLUGIN_CHANNEL_ID)
                return await event.edit(f'**Modul berhasil dimuat!**\n__Untuk informasi tentang perintah dan penggunaan modul__ `.xscr {dosyaAdi}` __menulis.__')
...
async def premove(event):
    ...
    async for message in event.client.iter_messages(PLUGIN_CHANNEL_ID, filter=InputMessagesFilterDocument, search=modul):
        ...
        try:
            os.remove(f"./ironbot/modules/{message.file.name}")
...
async def psend(event):
    ...
    if os.path.isfile(f"./ironbot/modules/{modul}.py"):
        await event.client.send_file(event.chat_id, f"./ironbot/modules/{modul}.py", caption=LANG['IRON_PLUGIN_CAPTION'])
        await event.delete()
    else:
        await event.edit(LANG['NOT_FOUND_PLUGIN'])
...
async def ptest(event):
    ...
    await event.edit(LANG["DOWNLOADING"])
    if not os.path.exists('./ironbot/temp_plugins/'):
        os.makedirs('./ironbot/temp_plugins')
    dosya = await event.client.download_media(reply_message, "./ironbot/temp_plugins/")
    ...
    try:
        ...
        await event.edit(f"{LANG['PLUGIN_BUGGED']} {e}`")
        return os.remove("./ironbot/temp_plugins/" + dosya)

    return await event.edit(f'**Modul berhasil dimuat!**\
    \n__Anda dapat melakukan percobaan modul Anda. Plugin tidak akan berfungsi saat Anda memulai ulang bot.__')