...
from ironbot import LOGS, CMD_HELP, TEMP_DOWNLOAD_DIRECTORY, BOT_USERNAME
...
def time_formatter(milliseconds: int) -> str:
    ...
    tmp = ((str(days) + f" {LANG['DAY']}, ") if days else "") + \
        ...
        ((str(milliseconds) + " millidetik, ") if milliseconds else "")
    return tmp[:-2]
...
async def upload(u_event):
    ...
    if input_str in ("ironbot.session", "config.env"):
...
async def uploadas(uas_event):
    await uas_event.edit("Mohon tunggu...")
            ...
            if supports_streaming:
                ...
                await uas_event.client.send_file(
                    ...
                    progress_callback=lambda d, t: asyncio.get_event_loop(
                        ...
                        progress(d, t, uas_event, c_time, "Mengunggah...",
                                 file_name)))
            elif round_message:
                c_time = time.time()
                await uas_event.client.send_file(
                ...
                await uas_event.edit("TBD: Saat ini tidak berlaku.")
                return
            os.remove(thumb)
            await uas_event.edit("Unggahan berhasil !!")
...
CmdHelp('updown').add_command(
    'download', '<nama file tautan> (atau menjawab sesuatu)', 'Sunucuya dosyayı indirir.'
).add_command(
    'upload', '<jalur file di server>', 'Mengunggah file dari server Anda untuk mengobrol.'
).add_command(
    'wupload', ' <balas file tersebut> anonfiles|transfer|filebin|tmpninja|anonymousfiles|megaupload|bayfiles|letsupload|vshare', 'Mengunggah ke situs web pilihan Anda.'
).add_command(
    'unzip', '<tanggapan>', 'Ekstrak file Zip yang Anda tanggapi.'
).add_command(
    'uploadir', '<map>', 'Memuat seluruh folder.'
).add()